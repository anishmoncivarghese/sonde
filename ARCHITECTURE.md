...
Describes commit e044a961d7c4423c1ff0f138acd3907400bf7a6b plus uncommitted changes.

28 all-test module(s) were excluded. Run `sonde doc --include-tests` to include them.

Cross-module references by how they were resolved: COMPILER 724, LEXICAL 284, HEURISTIC 1821.
...
227 lower-weight dependency pair(s) are not shown in the diagram; the table below is complete.

| From | To | References | Evidence |
|---|---|---:|---|
| `src/resolve` | `src/adapters` | 69 | COMPILER 65, LEXICAL 3, HEURISTIC 1 |
...
| `src/cli` | `src/store` | 47 | COMPILER 29, LEXICAL 6, HEURISTIC 12 |
...
| `src/doc` | `src/repo` | 16 | COMPILER 12, LEXICAL 2, HEURISTIC 2 |
...
| `src/cli` | `src/doc` | 16 | COMPILER 9, LEXICAL 3, HEURISTIC 4 |

65 lighter dependencies are not listed. Run `sonde doc --module <path>` for a module's complete set.
157 further module pair(s) share symbol names but have no resolved reference between them. They are not shown: a shared name is a coincidence, not a dependency, and modules with parallel structure produce the most of them.
...
| `src/cli` | 4 | 133 | — |
| `src/doc` | 3 | 155 | `structuralBody`, `DOC_PATH`, `NoDocumentableModulesError`, `action`, `generateDoc`, `renderModuleDetail`, +2 more |
...
| `src/store` | 4 | 182 | `Store`, `Db`, `SymbolKind`, `migrate`, `openDb`, `filePath`, +53 more |
